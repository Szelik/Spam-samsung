...
function addDrop() {
    var mainClass = 'drop',
        ...
        leftPos = randomNumber(0, 100),
        topPos = randomNumber(0, 50);
    ...
    setTimeout(function () {
        newDrop.classList.add(styleDrop, doneClass, dropSize);
    }, 100);
    setTimeout(function () {
...
function startRaining() {
    if (raining) {
        ...
        rainTimer = setInterval(addDrop, 80);
        raining = true;
...
scroll.oninput = function () {
    ...
    if (scroll.value >= 3 && scroll.value <= 23) {
        startRaining();
    } else {
        stopRaining();
    }
